...
public class DOStaticListIfHaveThenNone extends DOBaseForm {

	public DOStaticListIfHaveThenNone() {
		super();
	}
	...
	public String getHtmlCode(DOIModel iModel) {
		...
		/*
		 * 第一服务查询已有的数据，用别名flag，配置则用所有的数据。
		 * 
		 * 用所有的数据除去已有的数据，则得到需要增加的数据。
		 * */
		
		
		List list = StringUtil.getStaticList(fm.getInputConfig());	
		
		DOService service = fm.getLinkService();
		List<BOInstance> listBO = service.invokeSelect();
		if(listBO != null) {
			for(int i = 0; i < listBO.size(); i++) {
				BOInstance bo = listBO.get(i);
				String backup1 = bo.getValue("flag");
				if(backup1 != null) {
					for(int j = 0; j < list.size(); j++){
						String[] one = (String[])list.get(j);
						if(one != null && one.length >= 1) {
							String flag = one[0];
							if(backup1.equals(flag))
								list.remove(j);
						}
					}
				}
		...
		buffer.append("<input type='text' style='border:#B3B3B3 1px solid;'  onchange=\"if(this.value==''){this.previousSibling.value='';}\"'")
		...
		buffer
				.append(
						"<IMG  style='CURSOR: pointer;padding-bottom:2px;margin-left:-21px;' onclick=invokeStaticPopup(this")
