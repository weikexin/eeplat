...
public class WLogBuzhuXiujiaAll extends DOAbstractAction {
	public String excute() {

		// 需要接收的数据：查询条件
		String seltype = null;// 查询类型：默认为上一个月;
		...
		try {
			...
			DOService service = DOService
					.getService("cw_worklog_browse_allowance_ck_total");
		...
		if (users != null && users.size() > 0) {
			...
			if (seltype != null && seltype.trim().length() == 0)
				seltype = null;
			...
			try {
				...
				if (selname != null) {
					...
					sql = "select emp_uid from cw_worklog where year(wdate) = " + year
