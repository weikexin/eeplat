...
public class SelectBXByDeptNoOtherManage extends DOAbstractAction {
	...
	public String excute() throws ExedoException {
		try {
			...
			DOService linkser = DOService.getService("do_org_user_link_browse_otherdeptmanager");
			...
			DOService linkserdept = DOService.getService("do_org_user_link_browse_dept_by_form");
			String dept_uid =
				 DOGlobals.getInstance().getSessoinContext().getInstance().getFormInstance().getValue("wdept_uid");
			List<BOInstance> linkListdept = new ArrayList<BOInstance>();
			if(dept_uid != null && !"".equals(dept_uid.trim())) {
				linkListdept = linkserdept.invokeSelect(dept_uid);
				List<String> listempdept = new ArrayList<String>();
				if(linkListdept != null && linkListdept.size() >0) {
					for(int n = 0; n < linkListdept.size(); n++) {
						BOInstance bi = linkListdept.get(n);
						String emp_uid = bi.getValue("user_uid");
						if(emp_uid != null)
							listempdept.add(emp_uid);
					}
				}
...
//					bi.setBo(this.service.getBo());
					...
					if(uid != null && !listemp.contains(uid.trim()) && listempdept.contains(uid.trim())) {
