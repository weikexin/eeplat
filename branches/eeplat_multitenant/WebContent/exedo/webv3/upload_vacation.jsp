...
<%
	...
	try{
		/////创建一个目录：文件类型
		String fileType = "vacationdate";
		File aFile = new File(DOGlobals.UPLOAD_TEMP +  fileType);
		aFile.mkdir();
		...
		if(aFile.isDirectory()) {
			...
			for (int i = 0; i < filelist.length; i++) { 
				File delfile = new File(DOGlobals.UPLOAD_TEMP +  fileType + "/" + filelist[i]); 
	...
	//		+ "&fileName=" + URLEncoder.encode(Escape.escape(fileName),"utf-8") + "&blobColName="
	//		+ blobColName);
