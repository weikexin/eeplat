...
public class WLogBuzhuXiujiaDept extends DOAbstractAction {
	public String excute() {
		...
		try {
			...
			DOService service = DOService
					.getService("cw_worklog_browse_allowance_ck_dept");
		...
		if (users != null && users.size() > 0) {
			...
			try {
				...
				if (selname != null) {
					...
					sql = "select emp_uid from cw_worklog where wdept_uid = '" + seldept_uid
					+ "' and year(wdate) = " + year
					+ " and month(wdate)=" + month + " group by emp_uid ";
...
//											}
											...
											xjmoney = xjmoney + xc_money;
											xjdays = xjdays + xc_days;
										}
									}
								}
								...
								if(cw_type != null && ("xx".equals(cw_type) || "xj".equals(cw_type))){
									xjmoney = xjmoney + WLogBuzhuTongjiUtil.getQueshMoney(nowDays, wdate, subsidy, format, cw_type);
									xjdays = xjdays + nowDays;
								}
						...
						if(ifappstatus) {
							...
							if(intNowYear == intYear && intNowMonth == intMonth) {
								...
								if(intNowDay > day-1) {
									...
									// 计算缺省日期
									if(cw_type != null && ("xx".equals(cw_type) || "xj".equals(cw_type))){
										xjmoney = xjmoney + WLogBuzhuTongjiUtil.getQueshMoney(countDays, nextDay, subsidy, format, cw_type);
										xjdays = xjdays + countDays;
									}
