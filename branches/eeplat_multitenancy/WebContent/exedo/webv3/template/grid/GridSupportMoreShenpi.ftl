	...
	<table id='g${model.objUid}' class='tablesorter' border="0" cellpadding="1" cellspacing="1" >
		...
		<tbody>
			...
			<#list model.moreGridFormLinks as item>
				...
				<#if (item.nameColspan?exists && item.nameColspan == 0) >
				    <td  <#if item.noWrapValue>nowrap='nowrap'</#if> <#if (item.newLine || (item_index+1)==model.moreGridFormLinks?size || model.moreGridFormLinks[item_index+1].newLine )> </#if>><#if '${dataBind(data,item)}' ==''> ${item.htmlValue} </#if></td>
				<#else>
				    <td <#if item.noWrapLabel>nowrap='nowrap'</#if>> ${item.l10n}</td>
				    <td width="200px" <#if item.noWrapValue>nowrap='nowrap'</#if> <#if (item.newLine || (item_index+1)==model.moreGridFormLinks?size || model.moreGridFormLinks[item_index+1].newLine )> </#if>><#if '${dataBind(data,item)}' ==''> ${item.htmlValue} </#if></td>
			    </#if>
