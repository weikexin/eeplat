			...
			$.ajax({
				...
				success: function( html ) {
					...
					var all = $("<div></div>"),
							...
							idRegex  = new RegExp("\\bid=[\"']?([^\"'>]*)[\"']?");


					// data-url must be provided for the base tag so resource requests can be directed to the
					// correct url. loading into a temprorary element makes these requests immediately
					var isPage = pageElemRegex.test(html);
					if(isPage && RegExp.$1 && dataUrlRegex.test(RegExp.$1) && RegExp.$1) {
						redirectLoc = RegExp.$1;
					}
          
         if(isPage && RegExp.$1 && idRegex.test(RegExp.$1) && RegExp.$1) {
						 var pageId =  RegExp.$1;
						 if($("#" + pageId).size() > 0){
						 	   $("#" + pageId).remove();
						 	}
