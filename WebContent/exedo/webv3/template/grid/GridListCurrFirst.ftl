	...
	<table id='g${model.objUid}' class='tablesorter' border="0" cellpadding="1" cellspacing="1" >
		<thead>
		  <tr>
			...
			<#if model.NO><#--是否有数字序列-->
				<th  align='center' width='35px' class="{sorter: 'digit'}" nowrap='nowrap'>序号</th>
			</#if>
	   ...
	   <#list data as ins>
			...
			<#if model.checkBox><#--定义CheckBox-->
				<td >&nbsp;&nbsp;<input type ='checkbox' value='${ins.uid}' class='list_check'  name='checkinstance'/> <input type ='hidden' value='${ins.uid}' name='checkinstance_hidden'/> </td>
			<#elseif model.radio>
...
//			$(this).find(".list_check").attr("checked",true);//点击就选中，容易出现问题
		...
		<#if (model.rowSize?exists && model.rowSize > 0 && pmlName?exists)>
			...
			//	$('#g2c90b0fd2ec7b577012ec7dbc2b70047 tbody  tr').eq(0).addClass("selected");
			//if($('#check_${model.objUid}').) {
				$('#g2c90b0fd2ec7b577012ec7dbc2b70047 tbody  tr').eq(0).click();
			//}
