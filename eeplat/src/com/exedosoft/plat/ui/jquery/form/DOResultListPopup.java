	...
	String getPopupForm(DOFormModel fm) {
		...
		buffer
				...
				.append(fm.getFullColID()).append("_show' id='").append(
						fm.getFullColID()).append("_show' class='").append(
						fm.getFullColID()).append("_show' ");
		...
		if (fm.getOnChangeJs() != null && !"".equals(fm.getOnChangeJs())) {
			...
			buffer.append("if($.browser.mozilla) $('#").append(
					fm.getFullColID()).append(
					"')[0].addEventListener('DOMAttrModified',function(){")
